...
import { API_ROOT, PATHS, SOCKET_URL } from '../../utils/constants';
...
const Header = () => {
  ...
  const handleNotificationClick = async (notif: any) => {
    ...
    const classId = typeof notif.attendanceId === 'object' ? notif.attendanceId?.classId : null;
    if (classId) {
      navigate(PATHS.STUDENT_ATTENDANCE.replace(':id', classId));
    }
  };
                ...
                <div className="absolute right-[-10px] top-full mt-2 w-80 bg-white rounded-xl shadow-lg border border-gray-100 overflow-hidden z-50">
                  ...
                  <div className="max-h-[320px] overflow-y-auto">
                    {notifications.length === 0 ? (
                      <div className="p-6 text-center text-sm text-gray-500">Không có thông báo nào.</div>
                    ) : (
                      <div className="flex flex-col border-t border-gray-100">
                        {notifications.map((notif: any) => (
                          ...
                          >
                            <div
                              className={`mt-1.5 w-2 h-2 rounded-full shrink-0 ${!notif.isRead ? 'bg-blue-600' : 'bg-transparent'}`}
                            />
