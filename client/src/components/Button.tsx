...
const Button = ({ variant = 'primary', icon, children, className = '', ...props }: ButtonProps) => {
  ...
  return (
    <button
      className={`cursor-pointer flex items-center gap-2 px-6 py-2 rounded-lg transition-all active:scale-95 font-semibold disabled:opacity-50 ${variants[variant]} ${className}`}
      {...props}
    >
