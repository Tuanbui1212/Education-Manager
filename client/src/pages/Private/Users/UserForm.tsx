...
const UserForm = () => {
  ...
  const selectedRoleName = roles.find((r: any) => r._id === formData.roleId)?.name?.toLowerCase();
  ...
  useEffect(() => {
    ...
    const initData = async () => {
      ...
      try {
        ...
        if (id) {
          // Chế độ chỉnh sửa: load dữ liệu user
          ...
          if (userRes.success && userRes.data) {
            ...
            setFormData({
              ...
              teacher_info: {
                type: d.teacher_info?.type || 'PART_TIME',
                hourlyRate: d.teacher_info?.hourlyRate || 0,
              },
          ...
          // Chế độ thêm mới: mặc định role Student
          const studentRole = rolesList.find((r: any) => r.name?.toLowerCase() === 'student');
          if (studentRole) setFormData((prev) => ({ ...prev, roleId: studentRole._id }));
        }
      } catch (error) {
        console.error(error);
      } finally {
  ...
  const handleStudentInfoChange = (field: string, value: any) => {
    setFormData({ ...formData, student_info: { ...formData.student_info, [field]: value } });
    if (errors[field]) setErrors({ ...errors, [field]: '' });
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Dọn dữ liệu theo role
      if (selectedRoleName === 'student') {
        delete submitData.teacher_info;
        delete submitData.bankInfo;
      ...
      <div className="max-w-5xl mx-auto">
        ...
        <div className="flex items-center gap-4 mb-8">
          ...
          <h2 className="text-2xl font-bold text-gray-800">
            {id ? 'Chỉnh sửa hồ sơ tài khoản' : 'Thêm người dùng mới'}
          </h2>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
            ...
            <div className="xl:col-span-2 space-y-6">
              ...
              {selectedRoleName === 'student' && (
                <div className="bg-white p-8 rounded-3xl shadow-sm border border-gray-100">
                  <h3 className="text-lg font-bold text-gray-800 mb-6 flex items-center gap-2 border-b pb-3">
                    <Users className="text-green-600" size={20} /> Thông tin Học viên
                  </h3>
                  <div className="space-y-6">
                    <InputField
                      label="Tên phụ huynh"
                    ...
                    <div>
                      <Combobox
                        ...
                        onSelect={(c) => handleStudentInfoChange('consultantId', c?._id)}
              ...
              {selectedRoleName !== 'student' && (
                <div className="bg-white p-8 rounded-3xl shadow-sm border border-gray-100">
                  <h3 className="text-lg font-bold text-gray-800 mb-6 flex items-center gap-2 border-b pb-3">
                    <CreditCard className="text-indigo-600" size={20} /> Thông tin Thanh toán
                  </h3>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                      <SelectField
          ...
          <div className="flex justify-end gap-4 mt-8 bg-white p-5 rounded-3xl shadow-sm border border-gray-100 sticky bottom-6 z-10">
            ...
            <Button
              ...
              icon={<Save size={18} />}
            >
              {isLoading ? 'Đang xử lý...' : id ? 'Lưu thay đổi' : 'Thêm người dùng'}
            </Button>
          </div>
        </form>
      </div>
