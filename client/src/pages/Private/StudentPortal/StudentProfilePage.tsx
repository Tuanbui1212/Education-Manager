...
const StudentProfilePage = () => {
  ...
  const inputCls = 'w-full px-4 py-3 text-sm border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-400 transition-all bg-white';

  return (
    <div className="animate-in fade-in slide-in-from-bottom-4 duration-500 max-w-2xl mx-auto">
      <div className="flex items-center gap-4 mb-8">
        <button onClick={() => navigate(PATHS.STUDENT_PORTAL)} className="p-2.5 bg-white border border-gray-200 rounded-xl hover:bg-gray-50 transition-colors shadow-sm">
          <ArrowLeft size={20} className="text-gray-600" />
        </button>
        <div>
          <p className="text-xs font-bold text-blue-600 uppercase tracking-widest mb-0.5">Học viên</p>
          <h1 className="text-2xl font-bold text-gray-900">Hồ sơ cá nhân</h1>
        </div>
      </div>

      {/* Avatar card */}
      <div className="bg-gradient-to-br from-blue-600 to-indigo-700 rounded-3xl p-6 text-white mb-6 relative overflow-hidden shadow-lg shadow-blue-600/20">
        ...
        <div className="relative z-10 flex items-center gap-5">
          ...
          <div>
            <p className="text-blue-100 text-sm font-medium mb-0.5">Học viên</p>
            <h2 className="text-2xl font-bold">{info.fullName || 'Chưa cập nhật'}</h2>
            <p className="text-blue-200 text-sm mt-1 flex items-center gap-1.5"><Mail size={13} /> {info.email}</p>
          </div>
        </div>
      </div>
      ...
      <div className="flex mb-6 bg-white border border-gray-200 p-1.5 rounded-2xl shadow-xs w-fit">
        {([['info', <User size={16} />, 'Thông tin cá nhân'], ['password', <Lock size={16} />, 'Đổi mật khẩu']] as const).map(([tab, icon, label]) => (
          <button key={tab} onClick={() => setActiveTab(tab as ProfileTab)}
            className={`px-5 py-2.5 rounded-xl text-sm font-bold transition-all flex items-center gap-2 ${activeTab === tab ? 'bg-blue-600 text-white shadow-md shadow-blue-600/20' : 'text-gray-600 hover:bg-gray-50'}`}>
            {icon} {label}
          </button>
        ))}
      </div>
      ...
      {activeTab === 'info' && (
        <div className="bg-white rounded-3xl border border-gray-100 shadow-sm p-8 animate-in fade-in duration-300">
          <h3 className="font-bold text-gray-800 text-lg mb-6 flex items-center gap-2"><User size={18} className="text-blue-600" /> Thông tin cá nhân</h3>
          {loading ? (
            ...
            <form onSubmit={handleSaveInfo} className="space-y-5">
              ...
              <button type="submit" disabled={saving} className="w-full py-3 bg-blue-600 hover:bg-blue-700 disabled:opacity-60 text-white font-bold rounded-xl flex items-center justify-center gap-2 transition-all active:scale-95 shadow-md shadow-blue-600/20">
      ...
      {activeTab === 'password' && (
        <div className="bg-white rounded-3xl border border-gray-100 shadow-sm p-8 animate-in fade-in duration-300">
          <h3 className="font-bold text-gray-800 text-lg mb-6 flex items-center gap-2"><Lock size={18} className="text-blue-600" /> Đổi mật khẩu</h3>
